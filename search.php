...
<body>
    ...
    if($order == '최신순' || $order == ''){
        $sql = "SELECT * from board where b_title like '%$search%' or b_contents like '%$search%'";
    }
    elseif($order == '조회순'){
        $sql = "SELECT * from board where b_title like '%$search%' or b_contents like '%$search%' order by viewnum desc";
    }
    else{
        $sql = "SELECT * from board where b_title like '%$search%' or b_contents like '%$search%' order by board.like desc";
    }
    ...
    <div class="container">
        <div class="board_top">
            <h1 class="login" class="board_title">게시판</h1>
            <a href="writing.php?board=<?=$board?>" class="board_writing">글쓰기</a>
        </div>
        <div class="board_second">
            <div class="find_info">
                <a href="search.php?search=<?=$search?>&order=최신순" class="" style="color: #a5a5a7">최신순</a>
                <span class="bar" aria-hidden="true">|</span>
                <a href="search.php?search=<?=$search?>&order=조회순" class="" style="color: #a5a5a7">조회순</a>
                <span class="bar" aria-hidden="true">|</span>
                <a href="search.php?search=<?=$search?>&order=추천순" class="" style="color: #a5a5a7">추천순</a>
            </div>
